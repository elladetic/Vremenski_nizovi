...
data <- data.frame(x = c(1:400),y = ts(as.numeric(df[1,])))
...
#provjera sezonalnosti
library(seastests)
fried(data$y, freq = 7)
fried(data$y, freq = 12)

kw(data$y, freq = 7)
kw(data$y, freq = 12)
#nulta je da nije sezonalan -> velika p value,  ne odbacujem H_0 

isSeasonal(data$y, freq = 7) #nije sezonalan
```
#neki blagi trend prema gore, nije stacionaran 
#nebi rekla da je ovo sezonalno
...
ggplot(data, aes(x = x)) + geom_line(aes(y = y,color="Stvarni podaci"), col="red") +geom_line(aes(y = Polinom_1,col="Polinom prvog stupnja"), linetype="dashed")  +geom_line(aes(y = Polinom_2,color="Polinom drugog stupnja"),linetype="dashed") +geom_line(aes(y = Polinom_3,color="Polinom trećeg stupnja"),linetype="dashed") + labs(color="Legenda")
...
ggplot(data[40:45, ], aes(x = x)) + geom_line(aes(y = y_no_trend, col="Stvarni podaci sa uklonjenim trendom")) +
geom_line(aes(y = y_fit_AR, col="Fitani AR(1) model"), linetype="dashed") +ylab("")+ labs(color="Legenda") +
geom_line(aes(y = y_fit_MA, col="Fitani MA(1) model"), linetype="dashed")
```
...
## izabrat ću GARCH model
```{r}
GARCH #koeficijenti su mu 1.419e+06  2.062e-01  3.311e-09  
```
...
library(fGarch)
KK = garchFit(formula = ~ garch(1, 1), data = data$y_no_trend)
predict(KK, n.ahead= 90, plot = TRUE)

new_t <- data.frame(t = 400)
fc_t = predict(GARCH, new_t) # u predict ovo mora ic ko dataframe iz nekog razloga da ga on prepozna
...
model = auto.arima(data$y_no_trend, seasonal = FALSE)
plot(data$x,data$y_no_trend,type="l")
...
ggplot(data_new, aes(x = x)) + geom_line(aes(y = y, col="Stvarni podaci sa uklonjenim trendom")) +
geom_line(aes(y = upper, col="Fitani GARCH(1) model"), linetype="dashed") +
  geom_line(aes(y = lower, col="Fitani GARCH(1) model"), linetype="dashed")
